...
=======
# Draws the board (squares). Uses the Board() class and object.draw(surface) to draw each square on the board, with Board() taking inputs of which square to draw(for image files), and the x and y position of each square
...
square_class_instances = []

for i in range(len(square_groups)):
    square = board_squares[i]
    board_squares[i] = bd_pc.Board(square, square_placement[i][0], square_placement[i][1])
    square_groups[i] = p.sprite.GroupSingle
    square_groups[i].add(board_squares[i])


square_group = p.sprite.Group()
for i in range(len(board_squares)):
    square = board_squares[i]
    new_square = bd_pc.Board(square, square_placement[i][0], square_placement[i][1])
